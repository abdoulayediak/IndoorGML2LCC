...
void remove_aligned_points(vec_pt3d &pts)
{
...
str remove_irrelevant_around_string(str mystr, str toclean)
{
...
str remove_from_string(str mystr, str toclean)
{
    str res;
...
std::vector<double> split_string_delimiter_double(str mystr, str delim)
{
...
str_pair getNodeAttribute( rapidxml::xml_node<> *node, str name )
{
    str_pair res;
...
// Returns the Dart_handle of a face (SurfaceMemvber) of a CellSpace
Dart_handle getSurfaceMember(LCC& alcc, rapidxml::xml_node<> *&sm)
{
...
Dart_handle getCellSpaceGeometry(LCC& alcc, rapidxml::xml_node<> *&CS_chld)
{
...
//            std::cout<< "\tGot a surfaceMember!" << std::endl;
            ...
            {
                Dart_handle d = getSurfaceMember(alcc, sm);
...
CellSpace getCellSpace(LCC& alcc, rapidxml::xml_node<> *&CS)
{
    CellSpace res;
...
//        std::cout << "\tName: " << CS_chld->name() << " Value: " << CS_chld->value();
        ...
        else if (std::strcmp (CS_chld->name(), "core:partialboundedBy") == 0)
        {
            ...
            if ( *(xlink.begin()) == '#'  )
                xlink.erase(0,1); // Get rid of the "#"
            std::map<std::string, bool, cmp_string> bounded;
            bounded[ xlink ] = true;
...
void getPrimalSpaceFeatures_node( LCC& alcc, IndoorFeatures& InFt, rapidxml::xml_node<> *&pSF )
{
...
State getStateMember(rapidxml::xml_node<> *&node_n)
{
    State st;
...
Transition getTransitionMember(rapidxml::xml_node<> *&node_e)
{
    Transition trs;
...
SpaceLayer getSpaceLayer( rapidxml::xml_node<> *SL )
{
    SpaceLayer res;
...
void getMultiLayeredGraph_node( IndoorFeatures& InFt, rapidxml::xml_node<> *&mLG )
{
...
void getIndoorFeatures_node( LCC& alcc, rapidxml::xml_document<> &root, IndoorFeatures& InFt )
{
...
void readIndoorGML(LCC& alcc, IndoorFeatures& InFt, str myfile)
{
